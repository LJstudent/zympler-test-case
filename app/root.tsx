...
export function Layout({ children }: { children: React.ReactNode }) {
  return (
    <html
      lang="en"
      className="min-h-full scroll-smooth subpixel-antialiased js-focus-visible touch-manipulation bg-slate-600"
    >
      <head>
        <meta charSet="utf-8" />
      ...
      <body className="min-h-full bg-linear-to-b from-slate-700 to-slate-600 text-slate-800">
        <div className="flex flex-col min-h-dvh h-full p-2.5 gap-2.5 md:pb-15">
          ...
          <div className="relative flex justify-between items-stretch gap-2.5">
            ...
            <NavButton>
              <FaBell className="flex-none h-4.5 w-4.5" />
            </NavButton>
            <NavButton>
              <FaMoon className="flex-none h-4.5 w-4.5" />
            </NavButton>
            <NavButton>
              <FaUser className="flex-none h-4.5 w-4.5" />
            </NavButton>
          </div>
          ...
          <div className="w-full md:w-fit h-10">
            <div className="relative flex items-center justify-center gap-2.5 text-lg lg:text-2xl h-full font-semibold">
              ...
              <div className="relative flex items-center gap-2.5">
                <span className="max-w-full py-3 text-white">PowerBase</span>
                <span className="text-white">\</span>
                <span className="max-w-full py-3 text-white">Dashboard</span>
              </div>
            </div>
          </div>
            ...
            <div className="hidden md:inline flex-none w-10">
              <div className="flex flex-col gap-2.5 text-white">
                <NavButton active={true}>
                  <FaWaveSquare className="flex-none h-4.5 w-4.5" />
                </NavButton>
                <NavButton>
                  <FaMoneyBillTrendUp className="flex-none h-4.5 w-4.5" />
                </NavButton>
                <NavButton>
                  <FaEuroSign className="flex-none h-4.5 w-4.5" />
                </NavButton>
                <NavButton>
                  <FaLocationDot className="flex-none h-4.5 w-4.5" />
                </NavButton>
                <NavButton>
                  <FaCircleNodes className="flex-none h-4.5 w-4.5" />
                </NavButton>
              </div>
            </div>

            {/* content */}
            <div className="flex-1 flex flex-col gap-2.5 max-w-full min-w-0">{children}</div>

            {/* context bar */}
            <div className="hidden md:inline flex-none w-10"></div>
          </div>
        </div>
        <ScrollRestoration />
        <Scripts />
