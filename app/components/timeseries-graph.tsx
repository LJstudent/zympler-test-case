import { Bar, Cell, ComposedChart, DefaultZIndexes, Label, Line, ReferenceLine, ResponsiveContainer, useXAxisScale, useYAxisScale, XAxis, YAxis, ZIndexLayer } from "recharts";
...
import { DECIMAL_FORMAT, DefaultGraph, getSeriesKey, GraphType, PERCENTAGE_FORMAT, POWER_FORMAT } from "~/utils/graphs";
...
function get_from_object_per_tick_fn(resolution: TimeResolution): (local_date: DateTime, d: number) => DateTime {
  switch (resolution) {
    case 'day':
...
function get_floor_tick_fn(resolution: TimeResolution): (data_date: DateTime) => DateTime {
  switch (resolution) {
    case 'day':
      // truncate to 3 hours
      return (data_date: DateTime): DateTime => data_date.set({ hour: Math.floor(data_date.hour / 3) * 3, minute: 0, second: 0, millisecond: 0 })
  }
}
...
function get_x_ticks(resolution: TimeResolution, first_local_date: DateTime, last_local_date: DateTime): XTick[] {
  ...
  const from_object_per_tick_fn: (local_date: DateTime, d: number) => DateTime = get_from_object_per_tick_fn(resolution);
...
const usesNiceNumbers = (format: NumberFormat | undefined): boolean => {
  return format as NumberFormat ? FORMATS_WITH_NICE_NUMBERS.includes(format as NumberFormat) : false;
}
...
function get_y_ticks(minValue: number, maxValue: number, y_format: NumberFormat | undefined, {
  maxTicks = 6,
  paddingPctTop = 0.05,
  paddingPctBottom = 0.05,
} = {}): number[] {
...
function get_y_domain(y_ticks: number[], y_soft_zero: boolean, y_minimum?: number, y_maximum?: number): AxisDomain {
...
function get_wide_bars(first_local_date: DateTime, last_local_date: DateTime, bar_width: number, stack_bars: boolean): boolean {
...
function get_first_local_date(resolution: TimeResolution, local_combined_data: any[], x_property_name: string, timezone: string): DateTime {
  const first_local_date = (local_combined_data.length > 0 && _.get(local_combined_data[0], x_property_name) ? DateTime.fromISO(_.get(local_combined_data[0], x_property_name)) : DateTime.utc()).setZone(timezone);
...
function get_last_local_date(resolution: TimeResolution, local_combined_data: any[], x_property_name: string, timezone: string): DateTime {
  const first_local_date = (local_combined_data && local_combined_data.length > 0 && _.get(local_combined_data[0], x_property_name) ? DateTime.fromISO(_.get(local_combined_data[0], x_property_name)) : DateTime.utc()).setZone(timezone);
  const timespan = get_time_span(resolution);

  switch (resolution) {
    default:
      return getStartOfDate(resolution, getStartOfDate(resolution, first_local_date).plus(timespan));
  }
}
...
function get_interval_duration(resolution: TimeResolution, local_data: any[], x_property_name: string): Duration {
  const expected = Duration.fromObject(get_default_interval_duration(resolution));

  if (local_data.length > 1) {
    const actual = DateTime.fromISO(_.get(local_data[1], x_property_name)).diff(DateTime.fromISO(_.get(local_data[0], x_property_name)));
...
const newLabel = (x: number, y: number, value: number, format: NumberFormat | undefined, isForecast: boolean = false): LabelPosition => {
...
const repositionLabel = (position: LabelPosition, canvasSize: CanvasSize): LabelPosition => {
  ...
  const yOffset =
    aboveGraph ?
      ...
      canvasSize.height > 0 ?
        (canvasSize.height - position.arrowEndY - (LABEL_MINIMAL_Y_DISTANCE + labelHeight) - CANVAS_BOTTOM_MARGIN) / 1.5 :
        0;
  const dy = aboveGraph ?
...
const getArrowWidth = (position: LabelPosition): number => 6;
const getArrowHeight = (position: LabelPosition): number => getAboveGraph(position) ?
  position.arrowEndY - getLabelBottomY(position) - ARROW_MARGIN :
  getLabelTopY(position) - position.arrowEndY - ARROW_MARGIN;

const MinMaxLabel = React.memo((
  {
    label
  }:
    {
  ...
  const arrowWidth = getArrowWidth(label);
...
function get_y_max_fn(stackable_series: Serie[], non_stackable_series: Serie[], data: any[], showLimits: boolean): [max: number, index: number] {
  ...
  const stackable_data = data.map((d) => {
    // sum all the data key values for this data point
    const filtered_stackable_values: number[] = stackable_series.map((s) => _.get(d, s.data_key)).filter((v) => typeof v === "number");

    if (filtered_stackable_values.length === 0) {
      return Number.NEGATIVE_INFINITY;
    } else if (filtered_stackable_values.every((v) => v > 0) ||
  ...
  const non_stackable_data = data.map((d) => {
    return Math.max(...non_stackable_series.map((s) => _.get(d, s.data_key)).filter((v) => typeof v == "number"));
  }).filter((v) => typeof v == "number");
...
function get_y_min_fn(stackable_series: Serie[], non_stackable_series: Serie[], data: any[], showLimits: boolean): [max: number, index: number] {
  stackable_series = stackable_series.filter((s) => !s.is_limit || showLimits);
  non_stackable_series = non_stackable_series.filter((s) => !s.is_limit || showLimits);
  ...
  const non_stackable_data = data.map((d) => {
    return Math.min(...non_stackable_series.map((s) => _.get(d, s.data_key)).filter((v) => typeof v == "number"));
...
const matchDataKey = (a: string, b: string): boolean => {
  return a.replace(/power_max$|power$|energy$/g, '') === b.replace(/power_max$|power$|energy$/g, '');
}
...
export default function TimeseriesGraph({
  ...
  const [{
    ...
    y_label,
    y_base,
  }, setInternalProps] = useState<InternalProps>(DEFAULT_INTERNAL_PROPS);
  ...
  useEffect(() => {
    ...
    let i = 0, j = 0, k = 0;
    while ((!!data && i < data.length) || (!!forecast_data && j < forecast_data.length) || (!!background_data && k < background_data.length)) {
      ...
      const minTs = [
        ...
        forecastDataTs,
        backgroundDataTs,
      ].filter(ts => ts !== null && ts !== undefined).sort()[0];

      // Find the minimum ISO retrieved_at string (skip nulls)
      const minRetrievedAt = [
        d?.retrieved_at,
        f?.retrieved_at,
        b?.retrieved_at,
      ].filter(ts => ts !== null && ts !== undefined).sort()[0];
    ...
    const defaultPositiveColor = graph.defaultPositiveColor ? graph.defaultPositiveColor() : DefaultGraph.defaultPositiveColor();
    const defaultNegativeColor = graph.defaultNegativeColor ? graph.defaultNegativeColor() : DefaultGraph.defaultNegativeColor();
    ...
    const combined_series = graph.forecast_series && show_forecasts ? graph.series.concat(...graph.forecast_series) : graph.series;
    ...
    const stackable_series = graph.series.filter((s) => s.graph_type == GraphType.Bar && stack_bars);
    const stackable_forecast_series = graph.forecast_series ? graph.forecast_series.filter((s) => (s.graph_type == GraphType.Bar && stack_bars)) : [];

    const non_stackable_series = graph.series.filter((s) => !(s.graph_type == GraphType.Bar && stack_bars));
    const non_stackable_forecast_series = graph.forecast_series ? graph.forecast_series.filter((s) => !(s.graph_type == GraphType.Bar && stack_bars)) : [];
    ...
    const first_local_date = get_first_local_date(resolution, local_data, x_property_name, timezone);
    ...
    const is_today = (first_local_date.toISODate() == today.toISODate() && ["day", "week"].includes(resolution));
    ...
    const [y_max, y_max_index]: [number, number] = get_y_max_fn(stackable_series, non_stackable_series, local_data, true);
    const [y_max_forecasts, y_max_forecasts_index]: [number, number] = get_y_max_fn(stackable_forecast_series, non_stackable_forecast_series, local_forecast_data, true);
    const [y_min, y_min_index]: [number, number] = get_y_min_fn(stackable_series, non_stackable_series, local_data, true);
    const [y_min_forecasts, y_min_forecasts_index]: [number, number] = get_y_min_fn(stackable_forecast_series, non_stackable_forecast_series, local_forecast_data, true);

    // min/max excluding limits to use for min/max tooltips
    const [y_max_wo_limits, y_max_index_wo_limits]: [number, number] = get_y_max_fn(stackable_series, non_stackable_series, local_data, false);
    const [y_min_wo_limits, y_min_index_wo_limits]: [number, number] = get_y_min_fn(stackable_series, non_stackable_series, local_data, false);

    const maxValue = has_data && has_forecast_data ?
    ...
    const minValue = has_data && has_forecast_data ?
          ...
          >
            ...
            >
              <YAxis
                ...
                width="auto"
                tickFormatter={(value) => {
                  return usesNiceNumbers(y_format) ? format_intl_number(value / y_base as number, DECIMAL_FORMAT) : format_intl_number(value as number, y_format)
                }}
                label={{ value: usesNiceNumbers(y_format) ? y_label : '', angle: -90, position: 'insideLeft', offset: 10 }}
                ticks={y_ticks}
              ...
              <XAxis
                ...
                domain={[
                  first_local_date.toMillis(),
                  last_local_date.toMillis()
                ]}
                allowDataOverflow={true}
              ...
              <XAxis
                ...
                type="number"
                scale="time"
                dataKey={(x) => DateTime.fromISO(_.get(x, x_property_name)).toMillis() + bar_off_set}
                domain={[
                  first_local_date.toMillis(),
                  last_local_date.toMillis()
                ]}
                ...
                tickFormatter={value => get_tick_date_format_fn(resolution)(DateTime.fromMillis(value))}
              ...
              {y_ticks.map(y_tick => (
                <ReferenceLine
                  key={y_tick}
                  y={y_tick}
                  className={maxValue > 0 && minValue < 0 && y_tick === 0 ? "stroke-slate-200" : "stroke-transparent group-hover/graph:stroke-slate-200"}
                  stroke=""
                  strokeWidth={2}
                  xAxisId="line"
              ...
              {local_series.filter(s => s.graph_type == GraphType.Bar).map((s, i) => (
                <Bar
                  key={getSeriesKey(s)}
                  name={s.data_key}
                  dataKey={s.data_key}
                  ...
                  fillOpacity={(activeSerie === "" || matchDataKey(activeSerie, s.data_key)) ? 1 : 0.25}
                  strokeWidth={0}
                ...
                >
                  {!s.color && local_data?.map((entry, index) => {
                    const color = _.get(entry, s.data_key) >= 0 ? defaultPositiveColor : defaultNegativeColor;
                    return (
                      <Cell
                        key={index}
                        className={color}
              ...
              {local_series.filter(s => s.graph_type == GraphType.Line).map(s => (
                <Line
                  key={getSeriesKey(s)}
                  ...
                  className={s.color ?? defaultColor}
                  stroke=""
                  strokeOpacity={(activeSerie === "" || matchDataKey(activeSerie, s.data_key)) ? 1 : 0.25}
                  dot={false}
                  activeDot={false}
              ...
              {local_forecast_series.filter(s => s.graph_type == GraphType.Bar).map(s => (
                <Bar
                ...
                >
                  {!s.color && local_data?.map((entry, index) => {
                    const color = _.get(entry, `fc.${s.data_key}`) >= 0 ? defaultPositiveColor : defaultNegativeColor;
                    return (
                      <Cell
                        key={index}
                        className={color}
              ...
              {local_forecast_series.filter(s => s.graph_type == GraphType.Line).map(s => (
                <Line
                  key={`fc.${getSeriesKey(s)}`}
                  type={s.curve_type ?? "monotoneX"}
                  name={s.data_key}
              ...
              {has_data && <ZIndexLayer zIndex={DefaultZIndexes.scatter + 2}>
                <MinMaxLabelLayer
                  data={local_data}
                  y={y_max_wo_limits}
