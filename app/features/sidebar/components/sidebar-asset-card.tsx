...
export function SidebarAssetCard({
  ...
  const actionClassName =
    "block w-full cursor-pointer p-4 text-left focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-brand-blue";

  return (
    <Card
      className={`group overflow-hidden shadow-sm transition-[border-color,box-shadow,transform] duration-150 hover:-translate-y-0.5 hover:border-brand-blue-light hover:shadow-panel motion-reduce:transform-none motion-reduce:transition-none ${
        isActive ? "border-brand-blue-light ring-1 ring-brand-blue-light" : ""
      }`}
    >
      {to === undefined ? (
        <button type="button" className={actionClassName} aria-label={`Open ${title} details`}>
          {content}
        </button>
      ) : (
        <Link
